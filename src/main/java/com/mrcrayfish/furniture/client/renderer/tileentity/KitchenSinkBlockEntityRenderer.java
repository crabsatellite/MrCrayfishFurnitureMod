...
public class KitchenSinkBlockEntityRenderer implements BlockEntityRenderer<KitchenSinkBlockEntity> {
    ...
    private void drawFluid(KitchenSinkBlockEntity te, PoseStack poseStack, MultiBufferSource source, float x, float y, float z, float width, float height, float depth, int light) {
        ...
        consumer.addVertex(pose, x, y + height, z).setColor(red, green, blue, 1.0F).setUv(maxU, minV).setLight(light).setNormal(pose, 0.0F, 1.0F, 0.0F);
        consumer.addVertex(pose, x, y + height, z + depth).setColor(red, green, blue, 1.0F).setUv(minU, minV).setLight(light).setNormal(pose, 0.0F, 1.0F, 0.0F);
        consumer.addVertex(pose, x + width, y + height, z + depth).setColor(red, green, blue, 1.0F).setUv(minU, maxV).setLight(light).setNormal(pose, 0.0F, 1.0F, 0.0F);
        consumer.addVertex(pose, x + width, y + height, z).setColor(red, green, blue, 1.0F).setUv(maxU, maxV).setLight(light).setNormal(pose, 0.0F, 1.0F, 0.0F);
    }
}
