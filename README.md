![Furniture Banner](https://i.imgur.com/L03Tgig.png)

[![Download](https://img.shields.io/static/v1?label=&message=Download&color=2d2d2d&labelColor=dddddd&style=for-the-badge&logo=data:image/png;base64,)](https://mrcrayfish.com/mods?id=cfm) ![Minecraft](https://img.shields.io/static/v1?label=&message=1.15%20|%201.14%20|%201.12&color=2d2d2d&labelColor=dddddd&style=for-the-badge&logo=data:image/png;base64,) ![Curseforge](http://cf.way2muchnoise.eu/full_mrcrayfish-furniture-mod_downloads.svg?badge_style=for_the_badge)

# MrCrayfish's Furniture Mod — Continued

> **Unofficial Community Continuation (1.21.1 / NeoForge)**
>
> This is an unofficial community fork of [MrCrayfish's Furniture Mod](https://github.com/MrCrayfish/MrCrayfishFurnitureMod), maintained so that players can carry the furniture-rich builds they created on older Minecraft versions forward into newer ones without losing their creations.
>
> - **Target platform**: Minecraft **1.21.1** on **NeoForge 21.1.214**
> - **Goal**: Full API migration with save compatibility — the mod ID remains `cfm`, so existing worlds built on earlier versions keep their placed furniture intact.
> - **Scope**: Port only. No new features; behavior tracks upstream where the API allows.
> - **License**: [GPL-3.0](LICENSE), same as upstream. Original author: [MrCrayfish](https://github.com/MrCrayfish). Original copyright and credits are preserved in [CREDITS.txt](CREDITS.txt).
> - **Bug reports**: issues *specific to this fork or the 1.21.1 port* go to [this repository's issue tracker](https://github.com/crabsatellite/MrCrayfishFurnitureMod-Continued/issues). Please do **not** file port-specific issues upstream.

---

## Original README

This mod adds in what Minecraft has been missing for years, furniture! It includes over 80 unique pieces of furniture to decorate your bedroom, kitchen, living room and even your garden! Turn your house into the dream house you have never been able to have until now.

### Features:
* Includes over eighty unique pieces of furniture!
* Working functionality in some blocks, not just a decoration.
* A fully working mail system. Send items and blocks to players around your server!
